...
export function seaportEip712Message(args: WithCounter<OrderFragment>) {
  ...
  const tokenConsideration = {
    itemType: "4",
    token: args.contract,
    identifierOrCriteria: merkleTree(args.fulfillmentCriteria.token.identifier)
      .root,
    startAmount: "1",
    endAmount: "1",
    recipient: args.offerer,
  ...
  const feeConsiderationList = marketplaceFeeConsideration
    ? [marketplaceFeeConsideration]
    : [];
  ...
  return {
    ...
    consideration: [
      ...ethConsiderationList,
      ...feeConsiderationList,
      ...tokenConsiderationList,
    ],
    orderType: "0",
...
export function seaportFulfillAdvancedOrderArgs(
  ...
  const consideration = fulfillAdvancedOrderMessage.consideration.map((obj) =>
    Object.values(obj),
  );
