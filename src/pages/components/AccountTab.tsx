...
export function AccountTab({
  ...
  const [lastSelectedTokenId, setLastSelectedTokenId] = useState<
    number | undefined
  >(undefined);
  const {
    cancelAllOrders,
  ...
  useEffect(() => {
    ...
    if (seaportIncrementCounterStatus == "pending:receipt") {
      setDialog(
        CancelAllOrdersDialog("Waiting for transaction to confirm ..."),
      );
      return;
    }
  ...
  return (
    <Tab hidden={!showTab}>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {userUnlistedTokens.map((tokenId) => (
                      <CardNftSelectable
                        ...
                        onSelect={() =>
                          setSelectedTokenId(
                            selectedTokenId == tokenId ? undefined : tokenId,
                          )
                        }
                        tokenId={tokenId}
                      />
      ...
      >
        <Button
          disabled
        >{`${collection?.name} #${selectedTokenId || lastSelectedTokenId}`}</Button>
        <ButtonBlue
          onClick={() => {
    ...
    <div>
      <div className="flex flex-col items-center gap-4 max-w-lg">
        ...
        <div className="text-sx text-zinc-400 flex gap-2">
          <div>⚠️</div>
          <div>
            Warning: This will also cancel all your Opensea orders and offers,
            for all collections.
          </div>
        </div>
      </div>
